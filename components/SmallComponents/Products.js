...
const Products = ({
  ...
  console.log(rating);
  return (
    <div className="  cursor-pointer w-44 sm:w-72 sm:hover:scale-105 transition-all   p-5 shadow-xl rounded-2xl bg-white ">
      <p className=" hidden sm:block text-xs">{category}</p>
      <div className=" flex justify-center items-center mb-5 h-32 sm:h-40">
        <img className="  sm:my-4 mx-auto w-20 sm:w-28" src={image} alt={title} />
      </div>
      ...
      <div className=" active:scale-95 text-sm bg-black p-2 cursor-pointer text-white text-center  rounded-xl mt-3">
        Add to Cart
      </div>
    </div>
  );
